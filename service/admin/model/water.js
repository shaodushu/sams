...
const remove = async (req, res, next) => {
    try {
        ...
        const result = await operation.asyncHandleDbArgs(commands.water.remove, [new Date(), parseInt(id)])
        if (result.affectedRows === 1) {
...
const single = async (req, res, next) => {
    try {
        let param = tools.judgeObj(req.body) || tools.judgeObj(req.query) || tools.judgeObj(req.params), { id } = param;
        const result = await operation.asyncHandleDbArgs(commands.water.single, [parseInt(id)])
        if (result.length === 1) {
...
const update = async (req, res, next) => {
    try {
        ...
        const result = await operation.asyncHandleDbArgs(commands.water.update(key), [param.id])
        if (result.affectedRows === 1) {
